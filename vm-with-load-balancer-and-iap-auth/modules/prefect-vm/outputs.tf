...
output "prefect_url" {
  ...
  value       = var.enable_load_balancer ? "https://${var.environment}-${var.domain}" : "http://${google_compute_instance.vm_instance.network_interface[0].access_config[0].nat_ip}:4200"
}

output "iap_client_id" {
  description = "IAP OAuth client ID"
  value       = var.enable_load_balancer && var.enable_iap ? var.iap_client_id : null
  sensitive   = true
}
